          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors">
                ...
                Filters
                {(ownerFilter !== "all" || statusFilter !== "all" || excludeNoise) && (
                  <span className="ml-1 rounded-full bg-primary px-1.5 text-[10px] text-white">
                    {[ownerFilter !== "all", statusFilter !== "all", excludeNoise].filter(Boolean).length}
                  </span>
                )}
              </button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-72 p-3 space-y-3">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                />
                <Label htmlFor="exclude-noise" className="text-xs text-slate-600">Exclude noise</Label>
              </div>
            </PopoverContent>
          </Popover>
